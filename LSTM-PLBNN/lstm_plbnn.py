...
class NumpyLSTM:
    def __init__(self, input_size=4, hidden_size=32, output_size=1, learning_rate=0.001, prior_sigma=1.0, likelihood_sigma=0.1):
        ...
        for name in self.param_names:
            ...
            if not hasattr(self, "layer_rho"):
                self.layer_rho = {}
            
            self.layer_rho[name] = -5.0
    ...
    def sample_weights(self):
        ...
        for name in self.param_names:
            ...
            eps = np.random.randn()          # ONE random number
        ...
        for name in self.param_names:
            ...
            kl += (
                num_weights * (
                    ...
                    + (sigma**2)/(2*self.prior_sigma**2)
                    - 0.5
                )
            )
            
            kl += np.sum(mu**2)/(2*self.prior_sigma**2)
    ...
    def backward(self, y_pred, y_true, caches, kl_scale):
        ...
        for name in self.param_names:
            ...
            dmu = grads_w[name]
            dsigma = np.sum(grads_w[name]) * eps
        ...
        for epoch in range(1, epochs + 1):
            ...
            for x_seq, target in zip(X, y):
                ...
                for key, grad in grads.items():
                    ...
                    elif key.endswith("_layer_rho"):
                
                        name = key.replace("_layer_rho", "")
                
                        self.layer_rho[name] -= self.lr * grad
...
def main():
    ...
    def estimate_flops(input_size, hidden_size, output_size, seq_len, dataset_size, epochs, future_steps, uncertainty_samples=20, backward_factor=3):
        ...
        kl_ops = num_bayesian_layers * 8
        backward_bayesian_ops = num_bayesian_layers * 4
    ...
    try:
        out_path = Path("benchmark_results_lstm_plbnn.json")
